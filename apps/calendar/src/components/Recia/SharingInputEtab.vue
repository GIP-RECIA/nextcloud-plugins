...
<template>
	<NcSelect v-model="selected"
		:loading="loading"
		:options="etabs"
		:placeholder="t('calendar', 'Establishments')"
			...
			<li>
				<div class="option__selectall">
					<NcButton type="tertiary"
						size="small"
						:aria-label="t('calendar', 'Select all')"
						@click="selectAll">
						{{ t('calendar', 'Select all') }}
					</NcButton>
					<NcButton type="tertiary"
						size="small"
						:aria-label="t('calendar', 'Select none')"
						@click="deselectAll">
...
export default {
	...
	computed: {
		noResultText() {
			if (this.loading) {
				return t('calendar', 'Searching …')
			}
			return t('calendar', 'No elements found.')
		},
	},
...
.sharing-input-etab {
	...
	.vs__selected {
		...
		height: calc(var(--default-clickable-area) - 2 * var(--vs-border-width) - var(--default-grid-baseline)) !important;
	}
}
