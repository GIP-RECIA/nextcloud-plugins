...
class AdImporter
{
    ...
    {
        $date = date('Y-m-d');
        $uidAttribute = $this->appConfig->getValueString(
            Application::APP_ID,
            'cas_import_map_uid'
        );

        $displayNameAttribute1 = $this->appConfig->getValueString(
            Application::APP_ID,
        ...
        $emailAttribute = $this->appConfig->getValueString(
            Application::APP_ID,
            'cas_import_map_email'
        );
        $groupsAttribute = $this->appConfig->getValueString(
            Application::APP_ID,
            'cas_import_map_groups'
        );

        $groupsFilterAttribute = json_decode($this->appConfig->getValueString(
            Application::APP_ID,
            'cas_import_map_groups_fonctionel'
        ), true);
        $arrayGroupsAttrPedagogic = json_decode($this->appConfig->getValueString(
            Application::APP_ID,
            'cas_import_map_groups_pedagogic'
        ), true);
        $arrayRegexNameUai = json_decode($this->appConfig->getValueString(
            Application::APP_ID,
            'cas_import_map_regex_name_uai'
        ), true);

        $quotaAttribute = $this->appConfig->getValueString(
            Application::APP_ID,
            'cas_import_map_quota'
        );
        $enableAttribute = strtolower($this->appConfig->getValueString(
            Application::APP_ID,
            'cas_import_map_enabled'
        ));
        $dnAttribute = $this->appConfig->getValueString(
            Application::APP_ID,
            'cas_import_map_dn'
        );
        $mergeAttribute = boolval($this->appConfig->getValueString(
            Application::APP_ID,
            'cas_import_merge'
        ));
        $primaryAccountDnStartswWith = $this->appConfig->getValueString(
            Application::APP_ID,
            'cas_import_map_dn_filter'
        );
        $preferEnabledAccountsOverDisabled = boolval($this->appConfig->getValueString(
            Application::APP_ID,
            'cas_import_merge_enabled'
        ));
        $andEnableAttributeBitwise = $this->appConfig->getValueString(
            Application::APP_ID,
        ...
        if (sizeof($arrayGroupsAttrPedagogic) > 0) {
            foreach ($arrayGroupsAttrPedagogic as $groupsAttrPedagogic) {
                if (
                    array_key_exists("field", $groupsAttrPedagogic)
                    && strlen($groupsAttrPedagogic["field"]) > 0
                    && array_key_exists("filter", $groupsAttrPedagogic)
                    && strlen($groupsAttrPedagogic["filter"]) > 0
                    && array_key_exists("naming", $groupsAttrPedagogic)
                    && strlen($groupsAttrPedagogic["naming"]) > 0
                ) {
        ...
        $pageSize = $this->appConfig->getValueInt(
            Application::APP_ID,
            ...
            for ($key = 0; $key < $memberPage["count"]; $key++) {
                    ...
                    for ($j = 0; $j < $m[strtolower($groupsAttribute)]["count"]; $j++) {
                        ...
                        if (isset($m[strtolower(($groupsAttribute))][$j])) {
                            foreach ($arrayRegexNameUai as $regexNameUaiGroup) { # les regex associant le nom et  l'uai des etabs
                                if (
                                    array_key_exists("nameUai", $regexNameUaiGroup)
                                    && !is_null($regexNameUaiGroup["nameUai"])
                                ) {
                                    preg_match_all(
                                        '/' . $regexNameUaiGroup["nameUai"] . '/si',
                                        $resultGroupsAttribute,
                                        $uaiNameMatches,
                                        PREG_SET_ORDER,
                                        0
                                    );
                                    ...
                                    if (sizeof($uaiNameMatches) > 0 && sizeof($uaiNameMatches[0]) >= 2) {
                                        $indexRegexUaiGroup = array_key_exists("uaiGroup", $regexNameUaiGroup)
                                            && !is_null($regexNameUaiGroup["uaiGroup"])
                                            ? intval($regexNameUaiGroup["uaiGroup"])
                                            : null;
                                        $indexRegexNameGroup = array_key_exists("nameGroup", $regexNameUaiGroup)
                                            && !is_null($regexNameUaiGroup["nameGroup"])
                                            ? intval($regexNameUaiGroup["nameGroup"])
                                            : null;
                                        $uaiEtablissement = is_null($indexRegexUaiGroup)
                                            ? null
                                            : $uaiNameMatches[0][$indexRegexUaiGroup];
                                        $nameEtablissement = is_null($indexRegexNameGroup)
                                            ? null
                                            : $uaiNameMatches[0][$indexRegexNameGroup];

                                        $groupCnEtablissement = str_replace(
                                            "people",
                                            "structures",
                                            $this->appConfig->getValueString(
                                                Application::APP_ID,
                                        ...
                                        $uaiEtab = null;
                                        if (
                                            array_key_exists('entstructuresiren', $groupAttr)
                                            && $groupAttr['entstructuresiren']['count'] > 0
                                        ) {
                                ...
                                if (array_key_exists('filter', $groupFilter)) {
                                    if (
                                        ...
                                        if (array_key_exists('naming', $groupFilter)) {
                                            ...
                                            $groupName = $this->normalizedGroupeName(sprintf(
                                                $newName,
                                                ...$sprintfArray
                                            ));

                                            if (
                                                !is_null($groupFilterMatches)
                                                && !is_null($groupFilter["uaiNumber"])
                                                && count($assoEtablissementUaiOrNameAndId) > 0
                                            ) {
                                                ...
                                                !is_null($groupFilter["uaiNumber"])
                                                && !is_null($groupFilterMatches[intval($groupFilter["uaiNumber"])])
                                                && !array_key_exists(
                                                    $groupFilterMatches[intval($groupFilter["uaiNumber"])],
                                                    $assoEtablissementUaiOrNameAndId
                                                )
                                            ) {
                ...
                if ($addUser) {
                    foreach ($arrayGroupsAttrPedagogic as $groupsAttrPedagogic) {
                        if (
                            array_key_exists("field", $groupsAttrPedagogic)
                            && isset($m[strtolower($groupsAttrPedagogic["field"])][0])
                            && strlen($groupsAttrPedagogic["field"]) > 0
                            && array_key_exists("filter", $groupsAttrPedagogic)
                            && strlen($groupsAttrPedagogic["filter"]) > 0
                            && array_key_exists("naming", $groupsAttrPedagogic)
                                        ...
                                        foreach ($matches as $match) {
                                            ...
                                            if (is_numeric($match[1])) {
                                                ...
                                                if (strtolower($match[1]) === 'nometablissement') {
                                                    $sprintfArray[] = $this->getEstablishmentNameFromUAI(
                                                        $groupAttr['entstructureuai'][0]
                                                    );
                                                } elseif (
                                                    array_key_exists(strtolower($match[1]), $groupAttr)
                                                    && $groupAttr[strtolower($match[1])]["count"] > 0
                                                ) {
                ...
                if (isset($employeeID) && $addUser) {
                    ...
                    if ($userConfig->getValueString($employeeID, 'core', 'enabled') === 'false') {
                        $userConfig->setValueString(
                            $employeeID,
                            'core',
                            'enabled',
                            'true'
    ...
    {
        $qbHist = $this->dbConnection->getQueryBuilder();
        $qbHist
            ->select('uid')
            ->from('recia_user_history')
            ->where($qbHist->expr()->eq(
                'uid',
                $qbHist->createNamedParameter($uid)
            ));
        $uids = $qbHist->executeQuery()->fetchAll();

        return count($uids) > 0;
    }

    protected function saveUserHistory(
        $uid,
        $name,
        $isExists,
        $isAdded,
    ...
    {
        $qbAsso = $this->dbConnection->getQueryBuilder();
        $qbAsso
            ->select('id_etablissement')
            ->from('asso_uai_user_group')
            ->where($qbAsso->expr()->eq(
                'user_group',
                $qbAsso->createNamedParameter($uid)
            ));
        $etablissement = $qbAsso->executeQuery()->fetchAll();

        if (sizeof($etablissement) !== 0) {
            $idsEtab = array_unique(
                array_map(function ($asso) {
            ...
            foreach ($idsToDelete as $idToDelete) {
                ...
                $qbDelete
                    ...
                    ->where($qbDelete->expr()->eq(
                        'id_etablissement',
                        $qbDelete->createNamedParameter($idToDelete)
                    ))
                    ->andWhere($qbDelete->expr()->eq(
                        'user_group',
                        $qbDelete->createNamedParameter($uid)
                    ));
    ...
    protected function ldapConnect()
    {
        try {
            ...
            $this->ldapConnection = ldap_connect(
                ...
                ) . $host . ":" . $this->appConfig->getValueInt(
                    Application::APP_ID,
                    'cas_import_ad_port'
                )
