...
class UserService
{
    ...
    public function login($request, $uid, $password = '')
    {
        ...
        #\OCP\Util::writeLog('cas', 'phpCAS login function step 1.', \OCA\LdapImporter\Service\LoggingService::DEBUG);
        ...
        try {
            ...
            #\OCP\Util::writeLog('cas', 'phpCAS login function result: ' . $loginSuccessful, \OCA\LdapImporter\Service\LoggingService::DEBUG);
            ...
            #\OCP\Util::writeLog('cas', 'phpCAS login function not successful.', \OCA\LdapImporter\Service\LoggingService::DEBUG);
            ...
            #\OCP\Util::writeLog('cas', 'Owncloud could not log in the user with UID: ' . $uid . '. Exception thrown with code: ' . $e->getCode() . ' and message: ' . $e->getMessage() . '.', \OCA\LdapImporter\Service\LoggingService::ERROR);
    ...
    public function updateUser($user, $attributes)
    {
        ...
        #\OCP\Util::writeLog('cas', 'Updating data of the user: ' . $userId, \OCA\LdapImporter\Service\LoggingService::DEBUG);
        #\OCP\Util::writeLog('cas', 'Attributes: ' . $attributesString, \OCA\LdapImporter\Service\LoggingService::DEBUG);
        ...
        #\OCP\Util::writeLog('cas', 'Updating data finished.', \OCA\LdapImporter\Service\LoggingService::DEBUG);
    }
    ...
    private function updateMail($user, $email)
    {
        ...
        if ($email !== $user->getEMailAddress()) {
            ...
            #\OCP\Util::writeLog('cas', 'Set email "' . $email . '" for the user: ' . $user->getUID(), \OCA\LdapImporter\Service\LoggingService::DEBUG);
        }
    }
    ...
    private function updateName($user, $name)
    {
        ...
        if ($name !== $user->getDisplayName() && strlen($name) > 0) {
            ...
            #\OCP\Util::writeLog('cas', 'Set Name: ' . $name . ' for the user: ' . $user->getUID(), \OCA\LdapImporter\Service\LoggingService::DEBUG);
        }
    }
    ...
    public function updateGroups($user, $groups, $protectedGroups = '', $justCreated = false)
    {
        ...
        foreach ($groups as $group) {
            ...
            if (!$this->groupManager->isInGroup($uid, $group)) {
                ...
                #\OCP\Util::writeLog('cas', 'Added "' . $uid . '" to the group "' . $group . '"', \OCA\LdapImporter\Service\LoggingService::DEBUG);
            }
        }
    }
    ...
    public function updateQuota($user, $newGroupQuota, $quota = 'default')
    {
        ...
        $this->loggingService->write(\OCA\LdapImporter\Service\LoggingService::DEBUG, 'phpCas new UserQuota contents: ' . $quota . ' | New GroupQuota was: ' . $newGroupQuota);
